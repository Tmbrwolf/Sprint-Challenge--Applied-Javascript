class Tabs {
  ...
  getCards(data){
    ...
    if(data === 'all') {
      ...
      return document.querySelectorAll(`.card[data-tab='${data}']`); //weird editor function - closing prentheses missing on line 5
    }
  }
}
...
class TabLink {
  constructor(element, parent){
    ...
    this.cards = this.parent.getCards(this.element.dataset.tab); //key to learn this today & yesterday - original code
    ...
    this.cards = Array.from(this.cards).map(card => new TabCard(card)); //original code
  ...
  selectTab(){
    ...
    this.parent.updateActive(this); //original code
